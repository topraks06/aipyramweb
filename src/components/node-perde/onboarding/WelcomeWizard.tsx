...
export default function WelcomeWizard({ userUid, userName, profession, onComplete, basePath = '/sites/perde.ai' }: WelcomeWizardProps) {
  ...
  const getProfessionMessage = () => {
    switch (profession) {
      case 'perdeci': return 'Müşterilerinize oda fotoğrafında perde göstermeye hazır mısınız?';
      case 'ic_mimar': return 'Projelerinizi yapay zeka ile saniyeler içinde görselleştirin.';
      case 'mobilyaci': return 'Mekanlarınıza kumaş ve perde simülasyonu ekleyin.';
  ...
  const finishOnboarding = async () => {
    setIsFinishing(true);
    try {
      await updateDoc(doc(db, 'perde_members', userUid), {
