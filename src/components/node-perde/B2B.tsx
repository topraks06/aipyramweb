...
export default function B2B() {
  ...
  const { user, loading: authLoading, SovereignNodeId, isLicensed, logout } = useSovereignAuth('perde');
  ...
  const NAV_ITEMS = [
    { id: 'dashboard', label: 'HESABIM', icon: UserCircle },
    { id: 'projects', label: 'SİPARİŞLERİM', icon: Package },
    { id: 'profile', label: 'AYARLAR', icon: Compass },
  ];
      ...
      <aside className="w-64 bg-black/50 border-r border-white/10 flex flex-col justify-between backdrop-blur-xl shrink-0 z-20">
         <div className="p-6">
            ...
            <nav className="space-y-2">
               {/* TASARIM STÜDYOSU (B2C'de gizli veya farklı bir akışa sahip olacak) */}
         ...
         <div className="p-6 border-t border-white/10">
            ...
            <button onClick={async () => { await logout(); router.push('/sites/perde/login'); }} className="w-full py-3 text-[10px] font-bold flex justify-center items-center gap-2 text-zinc-400 hover:text-white border border-white/10 rounded-lg hover:bg-white/5 transition-colors uppercase tracking-widest">
                ...
                <div className="flex items-center gap-4">
                   ...
                   <div>
                      <h2 className="text-xl font-bold text-white tracking-wide uppercase flex items-center gap-2">
                        Perde.ai Dünyasına Hoş Geldiniz
                      </h2>
                      <p className="text-zinc-300 text-sm">Favori ürünlerinizi inceleyebilir, siparişlerinizin durumunu buradan takip edebilirsiniz.</p>
                   </div>
                </div>
             </div>
             ...
             <div className="flex flex-col md:flex-row justify-between border-b border-white/10 pb-6 mb-8 gap-6">
               <div>
                 <h1 className="font-sans font-bold uppercase tracking-widest text-3xl md:text-4xl text-white flex items-center gap-3">
                    <Compass className="w-8 h-8 text-blue-400" /> SİPARİŞLERİM & FAVORİLER
                 </h1>
                 <p className="text-zinc-400 text-sm mt-1">Tüm online siparişleriniz.</p>
               </div>
               <div className="flex flex-wrap gap-2 items-center">
                 <Link 
                    href="/sites/perde/collections"
                 ...
                 >
                    <Plus className="w-4 h-4" /> YENİ ALIŞVERİŞ
                 </Link>
               </div>
             </div>
