...
export default function Login({ basePath = '/sites/icmimar.ai' }: { basePath?: string }) {
  ...
  const { loginWithGoogle, loginWithEmail } = useSovereignAuth('icmimar');
  ...
  return (
    <AuthWrapper title="Sisteme Giriş" subtitle="Hesabınıza giriş yaparak B2B Yönetim Paneline erişin." basePath={basePath}>
      {error && (
      ...
      <div className="space-y-6">
        {/* Google Butonu - BİRİNCİL */}
        <button 
          type="button" 
          disabled={isProcessing}
          onClick={handleGoogleLogin}
          className="w-full bg-black text-white py-4 rounded-xl font-semibold text-sm hover:bg-zinc-800 hover:shadow-lg shadow-zinc-900/20 transition-all flex items-center justify-center gap-3 active:scale-[0.98]"
        >
      ...
      <form onSubmit={handleEmailLogin} className="space-y-5">
        ...
        {/* Form elements above have been preserved. This removes the duplicate google button. */}
