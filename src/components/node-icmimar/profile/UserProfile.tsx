...
export default function UserProfile({ basePath = '/sites/icmimar.ai' }: { basePath?: string }) {
  const { user, loading: authLoading } = useSovereignAuth('icmimar');
  ...
  useEffect(() => {
    ...
    async function fetchProfile() {
      try {
        const docRef = doc(db, 'icmimar_members', user!.uid);
  ...
  const handleUpdateProfile = async (e: React.FormEvent) => {
    ...
    setMessage(null);
    try {
      await updateDoc(doc(db, 'icmimar_members', user.uid), {
        name: formData.name,
        ...
        <section className="bg-white border border-[#111111]/10 shadow-sm p-8">
          ...
          <form onSubmit={handleUpdateProfile} className="space-y-6">
            ...
            <div>
              ...
              >
                <option value="perdeci">Icmimar Mağazası / Atölye</option>
