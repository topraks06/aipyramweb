...
export default function RoomVisualizer() {
  ...
  const lang = (searchParams.get('lang') || 'TR').toUpperCase() as keyof typeof ICMIMAR_DICT;
  const T = ICMIMAR_DICT[lang]?.visualizer || ICMIMAR_DICT['TR'].visualizer;
  const projectId = searchParams?.get('projectId');

  useEffect(() => {
    if (projectId) {
       fetch(`/api/icmimar/projects?id=${projectId}`)
         .then(res => res.json())
         .then(data => {
            if (data.project) {
  ...
  const { SovereignNodeId } = useSovereignAuth('icmimar');
  ...
  useEffect(() => {
    ...
    const handleRenderEdit = async (e: any) => {
      ...
      try {
        const response = await fetch('/api/icmimar/render-edit', {
          method: 'POST',
  ...
  const fetchArchives = async () => {
    try {
      const res = await fetch('/api/icmimar/projects');
           ...
           <div className="absolute inset-0 md:inset-2 flex items-center justify-center z-10">
               <div className="relative w-full h-full max-h-full bg-zinc-950 md:shadow-2xl md:rounded-2xl border border-white/5 overflow-hidden flex flex-col items-center justify-center">
                   ...
                   <div className="relative z-10 flex flex-col items-center p-4 md:px-6 md:pt-6 md:pb-4 backdrop-blur-xl bg-black/40 border border-white/10 rounded-2xl md:w-[38rem] w-[98%] max-h-[98%] text-center shadow-2xl mx-4">
                       ...
                       <h3 className="text-white text-xl font-medium tracking-tight mb-1">Icmimar Kumaşlarını Belirle</h3>
                                            ...
                                            <div className="flex bg-black/40 h-14 shrink-0 relative">
                                                ...
                                                <div className="flex-1 p-2 flex flex-col justify-center relative min-w-0">
                                                    <input 
                                                        ...
                                                        placeholder="Örn: Tül Icmimar, Fon Kumaşı..." 
                                                  ...
                                                  reader.onload = async (re) => {
                                                      ...
                                                      try {
                                                          const res = await fetch('/api/brain/v1/fabric-memory', {
                                                              method: 'POST',
                                                              headers: { 'Content-Type': 'application/json' },
                                                              body: JSON.stringify({ image_base64: base64, source_node: 'icmimar_visualizer' })
                                                          });
                                                          if (res.ok) {
                                                            const data = await res.json();
                 ...
                 <div className="absolute bottom-6 left-6 z-30 flex flex-wrap gap-2 items-center">
                     ...
                     <button 
                         onClick={() => {
                            ...
                            a.href = resultImage!;
                            a.download = `icmimar-ai-${crmData.projectName || 'tasarim'}-${Date.now()}.jpg`;
                            a.click();
                         }}
                     ...
                     <button 
                         onClick={() => {
                            window.open(`https://wa.me/?text=${encodeURIComponent('İcmimar.ai ile hazırladığım yeni tasarıma göz atın!')}`, '_blank');
                         }}
