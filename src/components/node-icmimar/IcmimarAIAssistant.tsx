...
export default function IcmimarAIAssistant() {
  const pathname = usePathname();
  const router = useRouter();
  const [locale, setLocale] = useState('tr');
  const d = DICT[locale] || DICT['tr'];
  const { SovereignNodeId, user } = useSovereignAuth('icmimar');
  ...
  const [currentSector, setCurrentSector] = useState('icmimar');
  ...
  useEffect(() => {
     if (pathname === '/sites/icmimar/schema/b2b' && !proactiveTriggered.current) {
  ...
  useEffect(() => {
    ...
    window.addEventListener('open_icmimar_ai_assistant', handleOpen);
    ...
    return () => {
        ...
        window.removeEventListener('open_icmimar_ai_assistant', handleOpen);
  ...
  const handleSend = async () => {
    ...
    if (isNavigating && (lower.includes('tasarım') || lower.includes('kanvas') || lower.includes('kaanvas') || lower.includes('çizim'))) {
      router.push('/sites/icmimar/visualizer');
    ...
    if (isColorAdvice) {
      ...
      try {
        const res = await fetch('/api/chat', {
          ...
          body: JSON.stringify({ message: `Kullanıcı iç mekan tasarımı için renk paleti önerisi istiyor: "${userMsg}". 60-30-10 kuralına göre 3 farklı renk paleti öner. Her palet için: ana renk, ikincil renk, aksan renk, hangi yüzeylere uygulanacağı. Kısa ve net cevap ver.`, history: [], sessionId: sessionId || `icmimar_${Date.now()}`, node: SovereignNodeId, authorId: user?.uid || 'anonymous' })
        });
    ...
    if (isInspirationIntent && !isNavigating && currentAttachments.length === 0) {
      ...
      try {
        const res = await fetch('/api/chat', {
          method: 'POST', headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ message: `İç mekan tasarımcısı olarak kullanıcıya ilham ver: "${userMsg}". 3-5 somut tasarım fikri öner. Her fikir için: stil adı, anahtar elemanlar (mobilya, renk, tekstil, aydınlatma), ve neden iyi olacağı. Icmimar, kumaş, mobilya, aydınlatma önerilerini dahil et. Kısa cevap.`, history: [], sessionId: sessionId || `icmimar_${Date.now()}`, node: SovereignNodeId, authorId: user?.uid || 'anonymous' })
        });
        const data = await res.json();
    ...
    if ((lower.includes('analiz') || lower.includes('incele')) && currentAttachments.length > 0) {
      ...
      fetch('/api/icmimar/analyze-fabric', {
        method: 'POST',
    ...
    if (lower.includes('koleksiyon') && (lower.includes('oluştur') || lower.includes('hazırla') || lower.includes('üret'))) {
      router.push('/sites/icmimar/studio');
    ...
    const keywords = ["halı", "aydınlatma", "kuaför", "berber", "mimar", "mobilya", "icmimar", "aksesuar", "bahçe", "peyzaj", "kumaş", "güzellik", "inşaat", "doktor", "oto", "diş", "restoran", "kafe", "yazılım"];
    ...
    try {
      const activeSessionId = sessionId || `icmimar_${Date.now()}`;
                                  ...
                                  <button 
                                      ...
                                      onClick={async (e) => {
                                         ...
                                         try {
                                             ...
                                             const newOrder = {
                                                SovereignNodeId: 'icmimar', // client context fallback
                                                authorId: 'seller', 
