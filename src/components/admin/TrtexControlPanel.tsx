    ...
    <div className="space-y-5">
      ...
      <div className="bg-[#0f0f18] rounded-xl border border-white/[0.06] p-4">
        <div className="flex items-center justify-between mb-3">
          <h2 className="text-sm font-bold text-white/90">⚡ Hızlı Aksiyonlar</h2>
          <a href="/sites/trtex.com" target="_blank" className="text-[10px] text-blue-400 hover:text-blue-300 flex items-center gap-1">
      ...
      {/* TOPIC MODAL */}
      {topicModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={() => setTopicModalOpen(false)}>
          <div className="bg-[#12121a] border border-white/10 rounded-xl p-6 w-full max-w-lg shadow-2xl" onClick={e => e.stopPropagation()}>
            <h3 className="text-sm font-bold text-white mb-3">📝 GÖRSEL GAZETECİLİK / Konu Yaz</h3>
            <p className="text-[11px] text-white/40 mb-4">Konu girin VEYA gerçek fuar/atölye fotoğrafları yükleyin. Aloha fotoğrafları görüp okuyarak gerçek B2B haber yazsın.</p>
            <input
              type="text"
              value={topicInput}
              ...
              className="w-full px-4 py-3 bg-white/[0.05] border border-white/10 rounded-lg text-sm text-white placeholder:text-white/30 focus:border-blue-500/50 focus:outline-none mb-3"
              autoFocus
            />
            
            {/* FILE UPLOAD INPUT */}
            <div className="relative border-2 border-dashed border-white/10 rounded-lg p-4 text-center hover:border-blue-500/30 transition-colors">
               <input 
                 type="file" multiple accept="image/*" 
                 onChange={e => setSelectedFiles(Array.from(e.target.files || []))}
                 className="absolute inset-0 w-full h-full opacity-0 cursor-pointer"
               />
               <ImageIcon className="h-5 w-5 text-white/30 mx-auto mb-2" />
               {selectedFiles.length > 0 ? (
                 <div className="text-xs font-bold text-emerald-400">{selectedFiles.length} GÖRSEL SEÇİLDİ</div>
               ) : (
                 <div className="text-[11px] text-white/40">Saha fotoğraflarını yükleyin (Opsiyonel)</div>
               )}
            </div>

            <div className="flex gap-2 mt-4">
              <button onClick={handleWriteTopic} disabled={!topicInput.trim() && selectedFiles.length === 0}
                className="flex-1 px-4 py-2.5 bg-blue-600 hover:bg-blue-500 disabled:opacity-30 text-white text-xs font-bold rounded-lg transition-colors">
              ...
              <button onClick={() => { setTopicModalOpen(false); setSelectedFiles([]); setTopicInput(""); }} className="px-4 py-2.5 bg-white/[0.05] text-white/60 text-xs rounded-lg hover:bg-white/10">
                İptal
              </button>
            </div>
            ...
            <div className="mt-4 flex flex-wrap gap-1.5">
              {[
                ...
                <button key={s} onClick={() => setTopicInput(s)}
                  className="text-[10px] px-2 py-1 bg-white/[0.04] hover:bg-white/[0.08] text-white/50 rounded border border-white/[0.06] transition-colors">
                  {s}
                </button>
              ))}
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        {/* STATS */}
        <div className="bg-[#0f0f18] rounded-xl border border-white/[0.06] p-4">
          <h3 className="text-xs font-bold text-white/70 uppercase tracking-wider mb-3">Sistem Durumu</h3>
          {stats ? (
            <div className="space-y-2.5">
              <div className="flex justify-between items-center">
                <span className="text-[11px] text-white/50">Toplam Haber</span>
                <span className="text-sm font-bold text-white">{stats.total}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-[11px] text-white/50">Görselli</span>
                <span className="text-sm font-bold text-emerald-400">{stats.withImages}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-[11px] text-white/50">Görselsiz</span>
                <span className={`text-sm font-bold ${stats.withoutImages > 0 ? 'text-amber-400' : 'text-emerald-400'}`}>{stats.withoutImages}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-[11px] text-white/50">Bugün Üretilen</span>
                <span className="text-sm font-bold text-blue-400">{stats.todayCount}</span>
              </div>
              <div className="border-t border-white/[0.06] pt-2 mt-2">
                <span className="text-[10px] text-white/30 uppercase tracking-wider">Kategoriler</span>
                <div className="flex flex-wrap gap-1 mt-1.5">
                  {Object.entries(stats.categories).map(([cat, count]) => (
                    <span key={cat} className="text-[10px] px-1.5 py-0.5 bg-white/[0.04] rounded text-white/50 border border-white/[0.06]">
                      {cat}: {count}
                    </span>
                  ))}
                </div>
            ...
            <div className="text-[11px] text-white/30">Yükleniyor...</div>
          )}
        </div>
        ...
        <div className="lg:col-span-2 bg-[#0f0f18] rounded-xl border border-white/[0.06] p-4 flex flex-col" style={{ maxHeight: '380px' }}>
          <h3 className="text-xs font-bold text-white/70 uppercase tracking-wider mb-2 flex items-center gap-1.5">
            <Zap className="h-3 w-3 text-blue-400" /> Aloha Chat — TRTEX Komutları
          </h3>
          <p className="text-[10px] text-white/30 mb-2">
            Komutlar: <code className="text-blue-400/70">haber yaz: konu</code> · <code className="text-blue-400/70">döngü başlat</code> · <code className="text-blue-400/70">ticaret modu</code>
          </p>
          ...
          <div className="flex-1 overflow-y-auto space-y-2 mb-2 min-h-[120px]" style={{ maxHeight: '220px' }}>
            {chatMessages.length === 0 && (
              <div className="text-center py-8">
                <p className="text-[11px] text-white/20">Aloha&apos;ya komut verin...</p>
                <div className="flex flex-wrap justify-center gap-1 mt-3">
                  {['haber yaz: Hometex Istanbul 2026 fuar analizi', 'döngü başlat', 'ticaret modu'].map(cmd => (
                    <button key={cmd} onClick={() => setChatInput(cmd)}
                      className="text-[10px] px-2 py-1 bg-blue-600/10 text-blue-400/70 rounded border border-blue-500/20 hover:bg-blue-600/20">
                      {cmd}
                    </button>
                  ))}
                </div>
            ...
            {chatMessages.map((m, i) => (
              <div key={i} className={`text-[11px] leading-relaxed px-3 py-2 rounded-lg ${
                m.role === 'user'
                  ...
                  : 'bg-white/[0.03] text-white/70 mr-4 border border-white/[0.04]'
              }`}>
                <span className="text-[9px] font-bold text-white/30 uppercase">{m.role === 'user' ? 'SEN' : 'ALOHA'}</span>
          ...
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 px-3 py-2 bg-white/[0.04] border border-white/[0.08] rounded-lg text-[12px] text-white placeholder:text-white/20 focus:border-blue-500/40 focus:outline-none"
      ...
      <div className="bg-[#0f0f18] rounded-xl border border-white/[0.06] p-4">
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-xs font-bold text-white/70 uppercase tracking-wider">📰 Haber Yönetimi ({filtered.length})</h3>
          <div className="flex items-center gap-2">
            <div className="relative">
              <Search className="h-3 w-3 absolute left-2.5 top-1/2 -translate-y-1/2 text-white/30" />
              <input
                value={searchQuery}
                onChange={e => setSearchQuery(e.target.value)}
                placeholder="Ara..."
                className="pl-7 pr-3 py-1.5 bg-white/[0.04] border border-white/[0.06] rounded-md text-[11px] text-white w-48 focus:outline-none focus:border-white/20"
              />
              {searchQuery && (
        ...
        {loading ? (
          ...
          <div className="text-center py-8 text-[11px] text-white/30">Haber bulunamadı</div>
        ) : (
          <div className="space-y-1">
            {/* Header */}
            <div className="grid grid-cols-12 gap-2 px-3 py-1.5 text-[9px] font-bold text-white/30 uppercase tracking-wider border-b border-white/[0.06]">
                ...
                {editingId === article.id ? (
                  ...
                  <div className="bg-blue-600/5 border border-blue-500/20 rounded-lg p-3 space-y-2">
                    <input value={editTitle} onChange={e => setEditTitle(e.target.value)}
                      className="w-full px-3 py-2 bg-white/[0.05] border border-white/10 rounded text-[12px] text-white focus:outline-none focus:border-blue-500/40" />
                    <textarea value={editSummary} onChange={e => setEditSummary(e.target.value)} rows={2}
                      className="w-full px-3 py-2 bg-white/[0.05] border border-white/10 rounded text-[11px] text-white/70 resize-none focus:outline-none focus:border-blue-500/40" />
                    <div className="flex gap-2">
                      <button onClick={saveEdit} className="px-3 py-1.5 bg-emerald-600 text-white text-[10px] font-bold rounded hover:bg-emerald-500 flex items-center gap-1">
                        <Check className="h-3 w-3" /> Kaydet
                      </button>
                      <button onClick={() => setEditingId(null)} className="px-3 py-1.5 bg-white/[0.05] text-white/50 text-[10px] rounded hover:bg-white/10">İptal</button>
                    </div>
                  </div>
                ) : (
                  /* VIEW MODE */
                  <div className="grid grid-cols-12 gap-2 px-3 py-2.5 rounded-lg hover:bg-white/[0.02] transition-colors items-center border-b border-white/[0.03]">
                    <div className="col-span-1 text-[10px] text-white/30 font-mono">{String(idx + 1).padStart(2, '0')}</div>
                    <div className="col-span-5">
                      <div className="text-[12px] font-medium text-white/80 truncate">{article.title}</div>
                      <div className="text-[10px] text-white/30 truncate mt-0.5">{article.summary}</div>
                    </div>
                    <div className="col-span-1">
                      <span className="text-[9px] px-1.5 py-0.5 bg-white/[0.04] rounded text-white/50 border border-white/[0.06]">
                        {article.category}
                      </span>
                    </div>
                    <div className="col-span-1">
                      <span className={`text-[11px] font-bold ${article.quality_score >= 80 ? 'text-emerald-400' : article.quality_score >= 60 ? 'text-amber-400' : 'text-red-400'}`}>
                        {(article.quality_score / 10).toFixed(1)}
                      </span>
                    </div>
                    <div className="col-span-1">
                      {article.image_generated || (article.images && article.images.length > 0) ? (
                        <span className="text-[10px] text-emerald-400">✅ {article.images?.length || 1}</span>
                      ) : (
                    ...
                    <div className="col-span-2 flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button onClick={() => startEdit(article)} title="Düzenle"
                        className="p-1.5 rounded hover:bg-blue-600/20 text-white/30 hover:text-blue-400 transition-colors">
                        <PenLine className="h-3 w-3" />
                      </button>
                      <button onClick={() => handleRegenImage(article.id, article.title)} title="Görsel Üret"
                        className="p-1.5 rounded hover:bg-purple-600/20 text-white/30 hover:text-purple-400 transition-colors">
                        <ImageIcon className="h-3 w-3" />
                      </button>
                      <a href={`/sites/trtex.com/news/${article.slug}?lang=tr`} target="_blank" title="Önizle"
                        className="p-1.5 rounded hover:bg-emerald-600/20 text-white/30 hover:text-emerald-400 transition-colors">
                        <Eye className="h-3 w-3" />
                      </a>
                      <button onClick={() => handleDelete(article.id, article.title)} title="Sil"
                        className="p-1.5 rounded hover:bg-red-600/20 text-white/30 hover:text-red-400 transition-colors">
