...
export default function HeimtexRegister({ lang = 'en' }: { lang?: string }) {
  return (
    <div className="min-h-screen bg-zinc-950 text-white flex flex-col font-sans">
      ...
      <main className="flex-1 flex items-center justify-center p-6">
        <div className="w-full max-w-md bg-zinc-900 p-8 rounded-xl border border-zinc-800">
          <h1 className="text-2xl font-serif uppercase tracking-widest text-center mb-8">Join Heimtex</h1>
          <form className="space-y-4">
            <div>
              <label className="block text-xs font-bold uppercase tracking-wider text-zinc-500 mb-2">Name</label>
              <input type="text" className="w-full bg-zinc-950 border border-zinc-800 p-3 text-white focus:border-red-500 outline-none transition-colors" />
            </div>
            <div>
          ...
          <div className="mt-6 text-center text-sm text-zinc-500">
            Already have an account? <a href="/login" className="text-white hover:text-red-500">Sign In</a>
          </div>
        </div>
      </main>
      <HeimtexFooter />
