...
> Son Güncelleme: 2026-04-20T08:20 | Yazar: Antigravity (Google-Native Migration)
...
Cache/Rate Limit: In-Memory (Google-Native — Upstash KALDIRILDI)
...
### Rate Limiting
Middleware'de in-memory rate limiter ile 10 saniyede 100 istek limiti. Map-tabanlı auto-cleanup.
...
### ~~Upstash Redis~~ → KALDIRILDI (20 Nisan 2026)
- **Anayasa kararı:** Sadece Google altyapısı kullanılacak.
- Rate limiting: In-memory Map tabanlı (middleware.ts)
- EventBus: Firestore event_signals koleksiyonu
- Queue: Firestore action_queue koleksiyonu
- Lock: Firestore system_locks + transactions
- İlgili tüm dosyalar Google-native'e çevrildi (11+ dosya)

### Gemini API
- `GEMINI_API_KEY` env gerekli
- ConciergeWidget → `/api/chat` üzerinden Gemini'ye çağrı yapar (fallback: lokal intent)
- ALOHA command → **Gemini 2.0 Flash destekli** intent resolution + tool executor
