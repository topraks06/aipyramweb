...
- ✅ **Firebase Genkit** — Eski custom `agentBus` yapısı çöpe atıldı, yerine native izlenebilir `simulateAgentFlow` (Genkit Flow) kuruldu.
- ✅ **Code Execution (B2B Hesap Makinesi)** — Python sandbox entegrasyonu tamamlandı. (Fiyat/metraj tahmini yok, kesin hesap var).
...
- ✅ İhale takibi, tedarik zinciri izleme gibi günlerce süren otonom görevler için **Long-running Agent** altyapısı kuruldu (`agentInbox.ts`).
- ✅ Agent Inbox (native) ile ajanlar arka planda görev işleyip sonucu Firestore `aloha_long_running_jobs` koleksiyonuna yazacak şekilde optimize edildi.
...
## 📊 Q2 2026 GEÇİŞ DURUMU (29 Nisan 2026 - GENKIT DEVRİMİ)
...
| Ajan İletişimi | Custom agentBus | **Firebase Genkit Flow** (`simulateAgentFlow`) | ✅ |
| B2B Hesaplamalar | Tahmin / Prompt | **Code Execution** (Python Sandbox) | ✅ |
| Hafıza Kapasitesi | Standart Limit | **2 Milyon Token Context Caching** | ✅ |
| Long-running Agents | 20s Timeout | **Asenkron Kuyruk** (`agentInbox.ts`) | ✅ |
| Flash TTS | — | Planlandı | ⬜ |
