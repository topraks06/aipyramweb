...
async function run() {
  const localImagePath = 'C:\\Users\\MSI\\.gemini\\antigravity\\brain\\0b537ced-8e34-416b-a8a7-ad75f6b92562\\para_hero_landscape_1776342141887.png';
  const destination = 'trtex-news/para_hero_landscape.png'; // New wide image

  console.log("Uploading wide image...");
  ...
  // Shift the old macro to images[2]
  if (existingImages[0]) {
    existingImages[2] = existingImages[0];
  }
  
  existingImages[0] = url; // new wide hero
  ...
  if (termConfig.exists) {
      ...
      if (data.heroArticle && data.heroArticle.id === docId) {
          ...
          data.heroArticle.images = existingImages;
      }
      if (data.gridArticles) {
          for(let g of data.gridArticles) {
              if (g.id === docId) {
                  g.image_url = url;
                  g.images = existingImages;
              }
          }
      }
  ...
  console.log("Firestore updated with the wide 16:9 Hero!");
  process.exit(0);
}
